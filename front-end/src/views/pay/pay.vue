...
<template>
  <div id="pay">
    ...
    <div class="order-info">
      <div class="avatar">
        <img :src="order_info.exRstLogoUrl">
      </div>
      <div class="info">
        <span class="price" style="font-size: 2em; font-weight: bold;">￥{{price}}</span>
        <p class="rst" style="margin-top: 0.7em;">{{order_info.exRstName}}</p>
        <p>订单号：{{order_id}}</p>
      </div>
    </div>
    <!--支付方式-->
  ...
  export default {
    ...
    methods: {
      ...
      submit() {              //提交支付
        ...
        initPay({order_id: this.order_id, payType: this.payType, method: this.method}).then((response) => {
          ...
          if (res.status === 200 || res.status === 90004) {   //判断该订单是否已经支付完成
            let _this = this;
            this.alertText = res.msg;      //提示
            this.showTip = true;
            setTimeout(() => {
    ...
    mounted() {
      ...
      orderInfo({order_id: this.order_id}).then((response) => {
        this.order_info = response.data.data.detail;
        console.log("订单信息");
        console.log(this.order_info);
        let endTime = new Date(this.order_info.invalidTime);
        let startTime = new Date();
        let remain_time = (endTime - startTime) / 1000;    //支付剩余时间
