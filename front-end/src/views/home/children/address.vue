        ...
        <li v-for="(item,index) in addressLists" :key="item.id">
          <div>
            <p>
              ...
              <span class="address_tag">{{item.tagName}}</span>
            </p>
            <span class="name">{{item.contact}}</span>
            <span class="phone">{{item.phone}}</span>
          </div>
          <i class="iconfont delete" v-show="status" @click="deleteAddress(item.shipAddrId,index)">&#xe615;</i>
        </li>
      </ul>
    </div>
  ...
  export default {
    ...
    created() {
      getAllAddress().then((response) => {
        this.addressLists = response.data.data.addresses;
        this.selectAddressId = this.addressLists[0].shipAddrId;
      })
    }
  }
</script>
