      ...
      <router-link
        ...
        <div class="detail">
          ...
          <div class="shops-message">
            <v-star :score="item.score"></v-star>
            <span class="sell-num">{{item.month_sale}}</span>
            <div class="delivery-info">
              <span class="deliver-time">{{item.delivery_time}}/</span>
              <span class="distance">{{item.distance}} m</span>
            </div>
          </div>
          <div class="price-message">
            <span>{{item.min_price}} | </span>
            <span>{{item.delivery_price}} | </span>
            <span>{{item.ave_consumption}}</span>
          </div>
  ...
  export default {
    ...
    methods: {
      ...
      getRestaurants(page, limit, callback) { //获取餐馆列表
        ...
        let lon = lng;
        console.log('- ajax');
        getRestaurants({from, size, lon, lat, type}).then((response) => {
          let data = response.data.data.rsts;
          this.preventRepeat = false;
          this.noMore = !data.hasNext;
          console.log(data.rstList);
          callback(data.rstList);
        });
      },
      firstFetch() {
        console.log('fetch');
        ...
        this.getRestaurants(this.page, this.limit, function (data) {
          console.log('exec callback, data: ' + data);
          _this.page++;
          _this.shopLists = data;
          console.log('this.shopLists: ' + _this.shopLists);
          _this.$nextTick(() => {
