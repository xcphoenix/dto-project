  ...
  export default {
    ...
    mounted() {
      ...
      getRestaurant({restaurant_id: _this.restaurant_id}).then((response) => {
        ...
        console.log('店铺信息: ' + _this.poi_info);
      })
    },
    methods: {
      fetchComment(callback) {   //获取评论
        restaurantComment({rstId: this.restaurant_id, page: {from: this.offset, size: 5}}).then((response) => {
          callback(response)
        })
      },
            ...
            if (!_this.loading) {   //避免加载过程中 重复请求
              ...
              _this.fetchComment(function (response) {
                if (!response.data.data.comments.length)
                  ...
                  _this.offset++;
                  _this.commentData = _this.commentData.concat(response.data.data.comments);
                  _this.$nextTick(() => {
