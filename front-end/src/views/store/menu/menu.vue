...
<template>
  <div id="menu">
    <div class="left" ref="left">
      <ul>
        <li v-for="(category,index) in foodsData" :key="category.categoryId" @click="mappingScroll(index)"
    ...
    <div class="right" ref="right">
      <article ref="categorys">
        <section v-for="category in foodsData" :key="category.categoryId">
          <h2>{{category.categoryName}}</h2>
          <article>
            <section v-for="spus in category.foodsDetail" :key="spus.foodId">
              <div class="img">
                <img :xsrc="spus.coverImg" src="../../../assets/shoploading.png">
              </div>
              <div class="info">
                <div class="name">{{spus.name}}</div>
                <div class="sell-num">月售 {{spus.monthSale}}</div>
                <span class="price"> ￥{{spus.sellingPrice}}</span>
              </div>
              <selector
                @showDot="showDotFun"
                :name="spus.name"
                :food_id="spus.foodId"
                :price="spus.sellingPrice"
                :pic="spus.coverImg">
              </selector>
            </section>
          </article>
  ...
  export default {
    ...
    created() {
      ...
      getFoods({restaurant_id}).then((response) => {
        this.foodsData = response.data.data.foods;
        console.log("获取食物 -> ");
