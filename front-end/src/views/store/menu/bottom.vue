...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      emptyCart() {
        console.log("清空购物车");
        clearCart(this.restaurant_id).then((response) => {
    ...
    watch: {
      ...
      poi_info(val) {  //监听vuex的reataurant_info信息
        this.shipping_fee = val.deliveryPrice;   //配送信息
        this.min_price = val.minPrice;         //最低价起送信息
        this.min_price_tip = val.minPrice;
        this.shipping_fee_tip = val.deliveryPrice;
      },
      "cartList": function () {
        console.log("购物车信息发生变化");
        let cartData = {};
        let foodItems = [];
        for (let p in this.cartList) {
          let obj = this.cartList[p];
          if (typeof obj == "object") {
            let food = {};
            food.original_price = obj.price;
            food.selling_price = obj.price;
            food.quantity = obj.num;
            food.food_id = obj.id;
            foodItems.push(food);
          }
        }
        cartData.cart_items = foodItems;
        cartData.original_total = this.totalPrice;
        cartData.total = this.totalPrice;
        cartData.totalWeight = this.foodNum;
        cartData.discountAmount = 0;
        cartData.restaurant_id = this.restaurant_id;
        
        console.log(cartData);
  
        submitCart(cartData).then((response) => {
          if (response.data.status === 200) {
            console.log("更新成功");
          } else {
            console.log("error");
