...
<template>
  <div id="order-detail">
    ...
    <section class="tip">
      <h3>{{orderStatus}}</h3>
      <router-link class="buy-again" :to="{path:'/store',query:{id:orderData.rstId}}" tag="span">再来一单</router-link>
    </section>
    <section class="foods-info-container">
      <div class="title">
        <span class="restaurant-picture">
          <img :src="orderData.exRstLogoUrl">
        </span>
        <span class="restaurant-name">{{orderData.exRstName}}</span>
      ...
      <div class="foods-container" v-for="(item,index) in foods" :key="index">
        ...
        <div class="main-container">
          <div>
            <span class="foods-name">{{item.exFoodName}}</span>
      ...
      <div class="other-fee">
        <div class="food-container-fee">
          <span class="fee-name">包装费</span>
          <span class="price">￥{{orderData.packagePrice}}</span>
        </div>
        <div class="delivery-fee">
          <span class="fee-name">配送费</span>
          <span class="price">￥{{orderData.deliveryPrice}}</span>
        </div>
      </div>
      <div class="total-price border-top">
        <span class="discount-price">优惠￥{{orderData.discountAmount}}</span>
        <span class="pay-price"> 实付 <strong>￥{{orderData.price}}</strong></span>
      </div>
    ...
    <section class="delivery-info-container">
      ...
      <div class="delivery-address">
        ...
        <div class="address-info">
          <span class="person-info item-value">{{orderData.exUserName}} - </span>
          <span class="phone item-value">{{orderData.exUserPhone}}</span>
          <span class="address item-value">{{orderData.exShipAddr}}</span>
        </div>
      </div>
    ...
    <section class="order-info-container">
      ...
      <div class="order-time">
        ...
        <span class="item-value">{{orderData.orderTime}}</span>
      </div>
  ...
  export default {
    ...
    created() {
      ...
      orderInfo({order_id: id}).then((response) => {
        ...
        let orderData = this.orderData = res.data.detail;
        this.orderStatus = orderData.statusDesc;
        this.foods = orderData.orderItems;
      })
    }
  }
</script>
