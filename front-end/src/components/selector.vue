...
//    接收父组件传递进来的货物  和 商店 id 和图片url
    ...
    methods: {
      addCart(event) {
        ...
        let {logo} = this.poi_info;   //商店名字 图片
        console.log(this.poi_info);
        let restaurant_name = this.poi_info.restaurantName;
        this.$store.dispatch('addCart', {
          ...
          restaurant_id: this.poi_info.restaurantId,
        });
        this.$emit('showDot', elRight, elBottom);
      },
      reduceCart() {
        this.$store.dispatch('reduceCart', {restaurant_id: this.poi_info.restaurantId, food_id: this.food_id})
      }
    },
    computed: {
      ...mapGetters([
      ...
      food_num() {
        let restaurant = this.cartList[this.poi_info.restaurantId];
